...
import { ApiResponse, exclude,getUserSearchOptions,getUserPaginationOptions,getUserSortOptions,  prisma } from '@/utils';
 ...
 * @access  Private
 */
export const updateMe = expressAsyncHandler(async (req:Request, res:Response):Promise<void> => {
  const { id } = req.user!;
 ...
 * @access  Private
 */
export const getMe = expressAsyncHandler(async (req:Request, res:Response):Promise<void>  => {
 ...
 */
export const getAllUsers = expressAsyncHandler(async (req:Request, res:Response):Promise<void>  => {
  const { page, limit, sort, search } = req.query;

  const { currentPage, itemsPerPage } = getUserPaginationOptions(page as string | undefined, limit as string | undefined);
  ...
  const response = new ApiResponse({
            messages: [{ message: 'Users retrieved Successfully', type: MessageType.SUCCESS }],
            data: users,
            pagination: {
 ...
 * @access  Private (admin)
 */
export const getUser = expressAsyncHandler(async (req:Request, res:Response):Promise<void>  => {
  const { id } = req.params;
